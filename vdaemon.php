...
// http://forum.x-code.com/
// vdaemon@x-code.com
//
///////////////////////////////////////////////////////////////////////////////

define('VD_VERSION', '3.0.10');
...
function VDGetCopyString()
{
	return '<span style="font-family: Verdana, Tahoma, Arial, Sans-Serif; font-size: 9px">Powered by '
		 ...
		 . '</a> &copy; 2002-2005 <a  href="http://www.x-code.com/index.php">X-code.com</a></span>';
}
...
function VDDirectCall()
{
	...
	&& isset($_POST['form']) && isset($_POST['post']) && isset($_POST['sesid']) && isset($_POST['hash'])) {
		...
		if (strcasecmp($sHash, md5($sVDaemonSecurityKey . $sSesID)) == 0) {
			...
			$_SESSION['VDaemonData']['POST'][$sForm] = array_merge($old_data, unserialize($sPost));
...
function VDValidate()
{
	...
	if (!$oVDaemonStatus->bValid || VDAEMON_SAVE_DATA) {
		if ($oRuntime->sDomain != $_SERVER['HTTP_HOST']) {
			...
			$_SESSION['VDaemonData']['POST'][$oRuntime->sForm] = array_merge($old_data, $_VDAEMON);
			if (!$oVDaemonStatus->bValid) {
...
class XmlNode
{
	...
	public function __construct($sName, $aAttrs = null, $nStart = 0, $nEnd = 0)
	{
		$this->sName = $sName;
		$this->nStart = $nStart;
		$this->nEnd = $nEnd;
...
class CVDForm
{
	...
	function AddControl(&$oNode)
	{
		if (isset($oNode->aAttrs['name']) && ($aPhpName = VDGetPhpControlName($oNode->aAttrs['name']))) {
			...
			$this->aControls[$aPhpName[0]]->AddInput($oNode, $aPhpName, $nIndex);
		}
	}
	...
	function SetInputErrClass($sFor, $sClass)
	{
		$sFor = VDHtmlDecode($sFor);
		foreach ($this->aControls as $sKey => $mTmp) {
			foreach ($this->aControls[$sKey]->aInputs as $nIdx => $oInput) {
				if ($oInput->nIndex >= 0 &&
				(isset($oInput->oNode->aAttrs['name'])
				&& VDHtmlDecode($oInput->oNode->aAttrs['name']) == $sFor) ||
				(isset($oInput->oNode->aAttrs['id'])
				&& VDHtmlDecode($oInput->oNode->aAttrs['id']) == $sFor)) {
					$this->aControls[$sKey]->aInputs[$nIdx]->oNode->aAttrs['class'] = $sClass;
				}
			}
		}
	}
...
class CVDSummary
{
	...
	public function __construct(&$oNode, $sId)
	{
		$this->oNode =& $oNode;
		$this->sId = $sId;
	}
...
class CVDControl
{
	...
	{
		$this->aInputs[] = new CVDInput($oNode, $aPhpName, $nIndex);
	}
...
class CVDInput
{
	...
	public function __construct(&$oNode, $aPhpName, $nIndex = -1)
	{
		$this->oNode =& $oNode;
		$this->nIndex = $nIndex;
